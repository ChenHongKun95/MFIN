...
class GlobalFeature(nn.Module):
    ...
    全局信息
    """
    def __init__(self, in_channels, channles, i, reduction=8):
        ...
        # ------------- local feature
        self.local_conv = nn.Conv2d(in_channels, channles, kernel_size=1)
        self.local_dconv = nn.Conv2d(in_channels, channles, kernel_size=3, stride=1, padding=i, dilation=i)
    ...
    def forward(self, x):
        batch, channel, height, weight = x.size()
         ...
         ####local feature
        local_1 = self.local_conv(x)
        local_2 = self.local_dconv(x)
        # -----------global
        # [B, C*1*1]
        ...
        out1 = torch.mul(x, avg_feature)
         ...
         #[B,C,1]
        gf = avg_feature.view(batch, channel, 1*1)
        gf = gf.unsqueeze(1)
        ...
        #[B, 1, H*W, C]
        x1 = input_x.permute(0, 1, 3, 2)
        #[B,1, H*W,1]
        conv_mask =  torch.matmul(x1,gf)
        
        conv_mask = conv_mask.squeeze(-1)
        # [B, 1, H*W]
        ...
        out2 = torch.mul(local_1, channel_mul)
        return out1 + out2
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.global_info1 = GlobalFeature(decode_channels, decode_channels, i = 7)
        self.global_info2 = GlobalFeature(decode_channels, decode_channels, i = 5)
        self.global_info3 = GlobalFeature(decode_channels, decode_channels, i = 3)
        self.global_info4 = GlobalFeature(decode_channels, decode_channels, i = 1)
